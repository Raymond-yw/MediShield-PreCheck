 ...
 * Updated for FHEVM 0.9 with official Sepolia addresses
 * Reference: https://docs.zama.org/protocol/solidity-guides/smart-contract/configure/contract_addresses
 */
async function initializeBrowserFheInstance() {
  ...
  // ✅ Step 1: Validate wallet network and auto-switch to Sepolia if needed
  try {
    const chainId = await window.ethereum.request({ method: 'eth_chainId' });
    const chainIdDecimal = parseInt(chainId, 16);
    console.log('🔍 Current wallet chainId:', chainIdDecimal, '(hex:', chainId, ')');
    
    if (chainIdDecimal !== 11155111) {
      console.warn('⚠️ Wallet not on Sepolia (11155111). Current:', chainIdDecimal);
      ...
      try {
        ...
        // If Sepolia is not configured in the wallet (error code 4902), try adding the network
        if (switchError.code === 4902) {
          console.log('⚠️ Sepolia network not found in wallet. Adding network...');
          try {
            await window.ethereum.request({
              ...
              params: [
                {
                  ...
                  rpcUrls: ['https://eth-sepolia.g.alchemy.com/v2/PdDY0FCflhQnCiLhEwxih'],
            ...
            throw new Error('Failed to add Sepolia network. Please add it manually in your wallet settings.');
          }
        } else {
      ...
      console.log('✅ Wallet is on Sepolia network');
    }
  } catch (err) {
    console.error('❌ Failed to verify/switch wallet network:', err);
    throw err;
  }

  // ✅ Step 2: Verify wallet accounts
  try {
    const accounts = await window.ethereum.request({ method: 'eth_accounts' });
    if (!accounts || accounts.length === 0) {
      throw new Error('No wallet accounts found. Please connect your wallet first.');
    }
    console.log('✅ Wallet account connected:', accounts[0]);
  } catch (err) {
    console.error('❌ Failed to verify wallet accounts:', err);
    throw err;
  }

  // ✅ Step 3: Check that RelayerSDK is available
  ...
  // ✅ Step 5: Use official Sepolia configuration
  ...
  console.log('Using official Sepolia FHEVM config:', config);
  
  // ✅ Step 6: Create the FHEVM instance
  try {
    fheInstance = await createInstance(config);
    console.log('✅ FHEVM instance created successfully with official config');
    return fheInstance;
  } catch (err) {
    ...
    throw new Error('Unable to initialize FHEVM. Please ensure you are connected to Sepolia network and try refreshing the page.');
  }
}
